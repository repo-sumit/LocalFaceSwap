...
LocalFaceFilter Native is a lightweight C++ webcam filter app designed for smooth realtime effects on lower-end machines. It uses a native OpenCV pipeline with face detection, optical-flow tracking, and simple overlay rendering instead of a heavy neural face-swap path.
...
- Optical-flow tracking between detector refreshes
...
- `--detect-interval=<frames>` tracked frames between full detector refreshes, default `4`
...
2. Detect faces infrequently and track in between.
...
3. Haar cascade finds the largest face when detection is needed.
4. `calcOpticalFlowPyrLK` tracks face motion between detections.
...
- use one face only
- increase `--detect-interval` to reduce detector cost
- keep debug mode off
...
- `native/assets/cascades/haarcascade_frontalface_default.xml` bundled face detector
